...
Flashcard sets and quizzes are generated by the LLM from a user-chosen PDF and persisted per user. Full payloads live in their own files under `Study_AI_users/{user_id}/{quizzes,flashcards}/<id>.json`; `user_data.json` only carries a lightweight summary index so it stays cheap to rewrite on every progress update. Chat history is **not** stored on the server — session memory keeps working the same way, but the transcript stays on the phone.
...
- `GET  /docs-list?user_id=<id>` — returns `[{filename, timestamp}, ...]` so the picker can show both.
...
`user_store.get_user_data()` fills in any missing top-level keys (e.g., `flashcard_sets`, `flashcards_revealed_total`) with safe defaults on every read, and writes the healed dict back only if something was added. Old accounts on disk heal themselves on first read — there is no one-off migration script to run.
